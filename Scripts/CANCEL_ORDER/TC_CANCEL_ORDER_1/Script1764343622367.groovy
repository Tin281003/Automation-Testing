...
// (Bắt buộc phải có đoạn này nếu chạy file lẻ, nếu không sẽ lỗi BrowserNotOpened)
...
WebUI.setText(txtUsername, "admin") // <-- Sửa lại username của bạn nếu khác

// Điền Password
TestObject txtPassword = new TestObject().addProperty("id", ConditionType.EQUALS, "password")
WebUI.setText(txtPassword, "12345678") // <-- Sửa lại password của bạn nếu khác
...
// --- BƯỚC 2: VÀO TRANG ĐƠN HÀNG ĐỂ HỦY ---
...
// Chờ kết quả tìm kiếm tải lại (quan trọng)
WebUI.delay(2) 
...
// Click nút option (dấu 3 chấm hoặc nút thao tác)
TestObject btnOption = new TestObject().addProperty("xpath", ConditionType.EQUALS, "//div[@id='main-order-content']/div/div/div/div[3]/div[5]/button")
WebUI.click(btnOption)

// Click Dropdown Status để mở danh sách chọn
TestObject ddlStatus = new TestObject().addProperty("id", ConditionType.EQUALS, "status")
WebUI.click(ddlStatus)

// Chọn giá trị "Canceled"
WebUI.selectOptionByLabel(ddlStatus, "Canceled", false)

// Confirm Update Shipping Status
...
// Chờ popup đóng hoặc trang load lại nhẹ
WebUI.delay(1)

// Click nút Cancel Order ở màn hình chi tiết
